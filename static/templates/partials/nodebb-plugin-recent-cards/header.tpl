      ...
      <a style="color: {topics.category.color};" href="{config.relative_path}/topic/{topics.slug}" itemprop="url">
        <div class="recent-card">
          ...
          <div class="topic-info" style="color: {topics.category.color};">
            <span class="h4" itemprop="description" style="color: {topics.category.color};">{topics.title}</span>
            <br>
            <span class="description">
              <strong>{topics.category.name}</strong>
              <br/>
              <span class="timeago" title="<!-- IF topics.teaser.timestampISO -->{topics.teaser.timestampISO}<!-- ELSE -->{topics.timestampISO}<!-- ENDIF topics.teaser.timestampISO -->"></span>
            </span>
          </div>
